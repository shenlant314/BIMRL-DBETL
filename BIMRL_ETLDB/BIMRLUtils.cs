   ...
   public static class BIMRLUtils
   {
      ...
      public static void updateMajorAxesAndOBB(int fedID, string whereCond)
      {
...
#endif
#if POSTGRES
         string sqlStmt = "SELECT ELEMENTID, geometrybody_geomtype, GEOMETRYBODY FROM " + DBOperation.formatTabName("BIMRL_ELEMENT", fedID) + " WHERE GEOMETRYBODY IS NOT NULL";
         if (!string.IsNullOrEmpty(whereCond))
            sqlStmt += " AND " + whereCond;
         ...
         while (reader.Read())
         {
            ...
            string geom = reader.GetString(2);
            object geombody = JsonGeomUtils.generateGeometryFromJson(geomType, geom);
            List<Polyhedron> polyHList = geombody as List<Polyhedron>;
      ...
      public static void UpdateElementTransform(IfcStore _model, string projectNumber, string projectName)
      {
         ...
         string sqlStmt = "SELECT FEDERATEDID FROM bimrl_federatedmodel WHERE MODELNAME = '" + modelName + "' AND PROJECTNUMBER='" + projectNumber + "' AND PROJECTNAME='" + projectName + "'";
#if ORACLE
         ...
         foreach (KeyValuePair<int,string> elemListItem in elemList)
         {
...
#if ORACLE
            sqlStmt = "update " + DBOperation.formatTabName("BIMRL_ELEMENT", fedID) + " set TRANSFORM_COL1=:1, TRANSFORM_COL2=:2, TRANSFORM_COL3=:3, TRANSFORM_COL4=:4"
            ...
            try
            {
               double[,] transf = new double[4, 4];
               transf[0, 0] = m3D.M11;
               transf[0, 1] = m3D.M12;
               transf[0, 2] = m3D.M13;
               transf[0, 3] = m3D.OffsetX * _model.ModelFactors.LengthToMetresConversionFactor;
               transf[1, 0] = m3D.M21;
               transf[1, 1] = m3D.M22;
               transf[1, 2] = m3D.M23;
               transf[1, 3] = m3D.OffsetY * _model.ModelFactors.LengthToMetresConversionFactor;
               transf[2, 0] = m3D.M31;
               transf[2, 1] = m3D.M32;
               transf[2, 2] = m3D.M33;
               transf[2, 3] = m3D.OffsetZ * _model.ModelFactors.LengthToMetresConversionFactor;
               transf[3, 0] = 0;
               transf[3, 1] = 0;
               transf[3, 2] = 0;
               transf[3, 3] = 1;
