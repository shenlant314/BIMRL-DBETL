...
namespace BIMRL_Main_Standalone
{
   /// <summary>
   /// Interaction logic for MainWindow.xaml
   /// </summary>

   public partial class BIMRL_ETLWindow : Window
   {
...
#if ORACLE
                _windowTitle = "BIMRL ETL Environment (Oracle edition)";
#endif
#if POSTGRES
               _windowTitle = "BIMRL ETL Environment (PostgreSQL edition)";
#endif
            return _windowTitle;
         }
         ...
         try
         {
            if (FedID >= 0)
            {
               ...
               else if (!regenSpatialIndex && regenBoundaryFaces && _majorAxes)
               {
                  ...
                  BIMRLUtils.ProcessFaceTask(FedID, whereCond, BIMRLCommonRef);
                  if (recreateIndex)
                     DBOperation.ExecuteSystemScript(FedID, "BIMRL_Idx_TopoFace_cr.sql", "BIMRL_Idx_MajorAxes.sql");
               }
